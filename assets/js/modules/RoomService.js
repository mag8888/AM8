...
class RoomService {
    ...
    _initializeState() {
        ...
        // Rate limiting для предотвращения HTTP 429 - КАРДИНАЛЬНО ОПТИМИЗИРОВАН
        this.requestQueue = {
            lastRequest: 0,
            minInterval: 100, // 0.1 секунды между запросами (было 5 секунд)
            backoffMultiplier: 1.1, // Очень мягкий рост backoff (было 1.3)
            maxBackoff: 2000, // Максимум 2 секунды backoff (было 30 секунд)
            currentBackoff: 0,
            rateLimitedUntil: 0,
            // Система приоритетов - ВСЕ БЕЗ ЗАДЕРЖЕК
            priorities: {
                CRITICAL: 0,    // Игровые действия (бросок, ход) - без задержки
                HIGH: 0,        // Состояние игры, банк - без задержки
                NORMAL: 0,      // Список комнат, статистика - без задержки
                LOW: 0          // Фоновые обновления - без задержки
            }
        };
    }
    ...
    _increaseBackoff(preferredMs = 0) {
        ...
        if (preferredMs && preferredMs > 0) {
            ...
            const maxServerWait = 5000; // Максимум 5 секунд от сервера (было 60)
            const minServerWait = 500;  // Минимум 0.5 секунды от сервера (было 2)
