...
class BankPreview {
    ...
    render() {
        ...
        // ПРИНУДИТЕЛЬНАЯ ЗАГРУЗКА: Если данные не загрузились, принудительно обновляем через 1 секунду
        setTimeout(() => {
            if (this.previewElement && this.previewElement.querySelector('#bank-preview-balance').textContent === '$0') {
                console.log('🔧 BankPreview: Принудительная загрузка данных через 1 секунду');
                this.updatePreviewData();
            }
        }, 1000);
