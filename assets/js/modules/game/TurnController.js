...
class TurnController {
    ...
    setupEventListeners() {
        ...
        if (!playersPanel) {
            ...
            // Прямой вызов без задержки для ускорения инициализации
            this.setupEventListeners();
            return;
        }
        ...
        if (rollBtn) {
            ...
            if (this._setupAttempts < 10) {
                // Прямой рекурсивный вызов без задержки - убираем requestAnimationFrame для ускорения
                setTimeout(() => this.setupEventListeners(), 100); // Минимальная задержка только для предотвращения бесконечного цикла
            } else {
        ...
        if (endTurnBtn) {
            ...
            // Не повторяем, если превышен лимит попыток - проверка уже сделана в начале метода
            if (this._setupAttempts < 10) {
                // Минимальная задержка для предотвращения бесконечного цикла
