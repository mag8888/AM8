...
class PlayersPanel {
    ...
    render() {
        if (!this.container) {
            console.error('❌ PlayersPanel: Контейнер не найден для рендеринга');
            return;
        }
        
        // ИСПРАВЛЕНО: Проверяем, есть ли уже отрендеренный контент
        const hasRenderedContent = this.container.querySelector('.game-right-panel');
        ...
        return players.map(player => {
            ...
            if (player.token) {
                if (typeof window.PlayerStatusUtils !== 'undefined' && typeof window.PlayerStatusUtils.getPlayerToken === 'function') {
                    playerToken = window.PlayerStatusUtils.getPlayerToken(player);
                } else if (/[\u{1F600}-\u{1F64F}]|[\u{1F300}-\u{1F5FF}]|[\u{1F680}-\u{1F6FF}]|[\u{1F1E0}-\u{1F1FF}]|[\u{2600}-\u{26FF}]|[\u{2700}-\u{27BF}]/u.test(player.token)) {
                    ...
                    playerToken = tokenIcons[player.token] || '👤';
                }
            }
    ...
    updateDiceResult(result) {
        ...
        if (isValid) {
            ...
            // НОВОЕ: Обновляем иконку кнопки "Бросок" с результатом кубика
            const moveBtn = document.getElementById('move-btn');
            if (moveBtn) {
                const btnIcon = moveBtn.querySelector('.btn-icon');
                if (btnIcon) {
            ...
            // Сбрасываем иконку кнопки "Бросок" на эмодзи, если нет результата
            const moveBtn = document.getElementById('move-btn');
            if (moveBtn) {
                const btnIcon = moveBtn.querySelector('.btn-icon');
                if (btnIcon) {
        ...
        // НОВОЕ: После обновления результата кубика обновляем кнопки
        setTimeout(() => {
            this.forceUpdateAllButtons();
        }, 100);
    ...
    updateControlButtons(state) {
        ...
        if (passBtn) {
            ...
            if (passBtnLabel && passBtnLabel.textContent !== 'Далее') {
