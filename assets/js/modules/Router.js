...
class Router {
    ...
    route(path, handler, title = '') {
        ...
        // Если это был ожидаемый маршрут, пытаемся его обработать
    ...
    executeRoute(path, state = {}) {
        const route = this.routes.get(path);
        if (!route) {
            // Не показываем ошибку для маршрутов, которые ещё могут быть зарегистрированы
            if (path === '/game' || path === '/rooms' || path === '/auth') {
                console.log(`🗺️ Router: Маршрут ${path} ещё не зарегистрирован, ожидаем инициализации`);
            } else {
                console.error(`❌ Router: Маршрут ${path} не найден`);
            }
            return;
    ...
    handleCurrentRoute() {
        ...
        // Если маршрут не зарегистрирован, переходим на default или ждем регистрации
        if (!this.routes.has(currentPath)) {
            // Для известных маршрутов пытаемся выполнить после регистрации
            if (currentPath === '/game' || currentPath === '/rooms' || currentPath === '/auth') {
                console.log(`🗺️ Router: Маршрут ${currentPath} будет обработан после регистрации`);
                // Пытаемся выполнить маршрут через небольшую задержку, если он уже зарегистрирован
                setTimeout(() => {
                    if (this.routes.has(currentPath)) {
                        console.log(`✅ Router: Маршрут ${currentPath} найден, выполняем`);
