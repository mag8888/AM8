#!/bin/bash

# Скрипт для настройки автоматического тестирования каждые 10 минут
# Запуск: ./setup-auto-test.sh

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CRON_JOB="0,10,20,30,40,50 * * * * cd $SCRIPT_DIR && node test-site.js >> site-test.log 2>&1"

echo "🔧 Настройка автоматического тестирования каждые 10 минут..."
echo ""

# Проверяем, существует ли уже такая задача в crontab
if crontab -l 2>/dev/null | grep -q "test-site.js"; then
    echo "⚠️  Задача уже существует в crontab"
    echo "Текущие задачи:"
    crontab -l | grep "test-site.js"
    echo ""
    read -p "Заменить существующую задачу? (y/n): " -n 1 -r
    echo ""
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        echo "❌ Отменено"
        exit 0
    fi
    # Удаляем старую задачу
    crontab -l 2>/dev/null | grep -v "test-site.js" | crontab -
fi

# Добавляем новую задачу
(crontab -l 2>/dev/null; echo "$CRON_JOB") | crontab -

echo "✅ Задача добавлена в crontab"
echo ""
echo "Расписание: каждые 10 минут (0, 10, 20, 30, 40, 50 минут каждого часа)"
echo "Логи сохраняются в: $SCRIPT_DIR/site-test.log"
echo ""
echo "Просмотр логов: tail -f $SCRIPT_DIR/site-test.log"
echo "Просмотр задач: crontab -l"
echo "Удаление задачи: crontab -e (удалите строку с test-site.js)"


